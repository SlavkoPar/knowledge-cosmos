...
const ChatBotDlg = ({ show, onHide }: IProps) => {
    ...
    const { getCatsByKind, getSubCats, getQuestion, addHistory, addHistoryFilter, getAnswersRated, searchQuestions, setLastRouteVisited } = useGlobalContext();
    ...
    useEffect(() => {
        (async () => {
            ...
            const subCats: ICat[] = await getSubCats(parentCategory);
            console.log('/////////////////////////////////////////////////////', subCats)
    ...
    const onOptionChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        const target = event.target;
        const { type, id, name } = target;
        const value = type === 'checkbox' ? target.checked : target.value;
        const level = parseInt(name as any);
        //let catLevel = catLevels[level];
        ...
        const subCats: ICat[] = await getSubCats(id);
        console.log('/////////////////////////////////////////////////////', subCats)
        setCatLevels((prevState) => ([
            ...prev,
            {
    ...
    const CatLevelComponent = (props: ICatLevel) => {
        const { level, catId, header, subCats, subCatSelected } = props;
        const subCats2 = subCatSelected !== null
            ? subCats.filter(c => c.id === subCatSelected!)
            : subCats;
        const marginLeft = (level * 10) + 'px';
        return (
            <Row
                className={`my-0 bg-warning text-dark mx-1 border border-1 rounded-1`}
                ...
                <Col xs={12} md={12}>
                    <div className="d-flex justify-content-start align-items-center" style={{marginLeft}}>
                        ...
                        <Form key='options' className='text-center border border-1 m-1 rounded-1'>
                            {!subCatSelected && header !== '' && <div className='text-center'>
                                {/* <i className='bg-secondary'> Select 'Mts' for test </i> */}
                                <i className='bg-secondary'>{header}</i>
                            </div>}

                            <div className='text-center bg-secondary'>
                                {subCats2.map(({ id, title, level }: ICat) => (
                                    <div key={id}>
                                        {subCatSelected && <span>{title}</span>}
                                        {!subCatSelected &&
    ...
    return (
        <div>
            ...
            <Offcanvas show={show} onHide={onHide} backdrop="static" placement='end'>
                    ...
                    <Container id='container' fluid className='text-info'> {/* align-items-center" */}
                        <Row className="my-1">
                            <Col className='border border-4 border-primary mx-1  bg-primary text-white'>
                                ...
                                <div className='border border-3 border-primary mx-1  bg-secondary text-white'>
