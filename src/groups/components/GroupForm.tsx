...
import { FormMode, ActionTypes, IGroupFormProps, IGroup, IVariation, IGroupKey } from "groups/types";
...
const GroupForm = ({ inLine, mode, group, submitForm, children }: IGroupFormProps) => {
  ...
  const { partitionKey, id, title, variations, answers, kind } = group;
  const groupKey: IGroupKey = { partitionKey, id }

  if (!document.getElementById('div-details')) {

  }
  const showAnswers = answers.length > 0 //!answers.find(q => q.inAdding);
  /* 
